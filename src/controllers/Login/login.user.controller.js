...
const BenhNhanRegisterService = require("../../utils/BenhNhanRegisterService");
module.exports = {
  ...
  registerBenhNhan: async (req, res) => {
    ...
    try {
      const check = await BenhNhan.findOne({ email });
      if (check) {
      ...
      //Factory pattern
      const newBenhNhan = await UserFactory.createUser("benhnhan", userData);
  ...
  getAccKH: async (req, res) => {
    try {
      ...
      let bn = await BenhNhan.find(query)
        .populate("roleId")
        .skip(skip)
        .limit(limitNumber);
  ...
  khoaAccKH: async (req, res) => {
    try {
      ...
      const updatedAccount = await BenhNhan.findByIdAndUpdate(
        id,
        { isActive },
        { new: true }
      );
  ...
  createKH: async (req, res) => {
    try {
      ...
      const service = new BenhNhanRegisterService();
  ...
  updateKH: async (req, res) => {
    try {
      let { _id, email, password, firstName, lastName, image, phone, address } =
        req.body;
      console.log("anhr: ", image);

      let createPhongKham = await BenhNhan.updateOne(
        { _id: _id },
        { address, email, password, firstName, lastName, image, phone }
      );
